...
#import "HUAMyController.h"
@interface HUAForgetPasswordController ()<UITextFieldDelegate>
{
...
@implementation HUAForgetPasswordController
    ...
    if (self.phoneNumberTF.text.length == 0 || self.verificationCodeTF.text.length == 0) {
        ...
        [manager GET:url parameters:parameter success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
            ...
            [HUAMBProgress MBProgressFromWindowWithLabelText:@"手机验证成功，正在跳转" dispatch_get_main_queue:^{
                HUAResetPasswordController *resetPasswordVC = [HUAResetPasswordController new];
                resetPasswordVC.old_password = responseObject[@"info"][0][@"password"];
                resetPasswordVC.phoneNumber = self.phoneNumberTF.text;
    ...
    if (![HUAMobileNumber isMobileNumber:self.phoneNumberTF.text]) {
        ...
        [manager GET:url parameters:parameters success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
            ...
            if ([[[[NSNumberFormatter alloc] init] stringFromNumber:responseObject[@"code"]] isEqualToString:@"3"]) {
                [HUAMBProgress  MBProgressFromWindowWithLabelText:@"未注册的手机号码"];
            }else {
