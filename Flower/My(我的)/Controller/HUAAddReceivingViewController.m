...
//
//  HUAAddReceivingViewController.m
//  Flower
//
...
@interface HUAAddReceivingViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation HUAAddReceivingViewController
...
- (void)ActionChange:(UIButton *)button{
    ...
    if (button.tag == 101) {
        ...
        NSArray *array =@[@{@"name":fiel.text,@"page":fiel2.text,@"address":_addressLabel.text,@"dizhi":fiel3.text}];
        

        
        if (fiel.text.length!=0 &&fiel2.text.length!=0  &&_addressLabel.text.length!=0 &&fiel3.text.length!=0){
            ...
            [HUAMBProgress MBProgressFromView:self.view wrongLabelText:@"请完善信息"];
            return;
        }
