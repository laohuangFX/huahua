...
@interface HUANewPasswordController ()<UITextFieldDelegate>
...
@implementation HUANewPasswordController
...
- (void)makeSure:(UIButton *)sender {
    if (self.firstPassWordTF.text.length == 0 || self.secondPassWordTF.text.length == 0) {
        ...
        [manager POST:url parameters:parameter success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
            ...
            }else if ([password isEqualToString:self.old_password]) {
                [HUAMBProgress MBProgressFromWindowWithLabelText:@"密码一样不需要修改"];
            }else {
                [HUAMBProgress MBProgressFromWindowWithLabelText:@"修改密码成功" dispatch_get_main_queue:^{
