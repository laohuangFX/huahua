...
// 选中颜色加深
#define SelectColor [UIColor colorWithRed:238.0f/255.0f green:238.0f/255.0f blue:238.0f/255.0f alpha:0.6]
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:HUAColor(0xf8f8f8)andPosition:bgLayerPosition];
        ...
        //NSLog(@"%@",titleString);
        self.titleName = [self createTextLayerWithNSString:titleString withColor:HUAColor(0x000000) andPosition:titlePosition];
        [self.layer addSublayer:self.titleName];
        [tempTitles addObject:self.titleName];
        

        
        //indicator
        CAShapeLayer *indicator = [self createIndicatorWithColor:[UIColor blackColor] andPosition:CGPointMake(titlePosition.x + self.titleName.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
        ...
        //separator
         if (i != _numOfMenu - 1) {
             ...
             CAShapeLayer *separator = [self createSeparatorLineWithColor:HUAColor(0xdddddd) andPosition:separatorPosition];
...
- (CAShapeLayer *)createSeparatorLineWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    UIBezierPath *path = [UIBezierPath new];
    [path moveToPoint:CGPointMake(161,0)];
    [path addLineToPoint:CGPointMake(161, self.frame.size.height/2)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        // 只取宽度
        textSize = [cell.textLabel.text textSizeWithFont:[UIFont systemFontOfSize:14.0f] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    if (leftOrRight == 1) {
        ...
        cell.backgroundColor = HUAColor(0xf2f2f2);
